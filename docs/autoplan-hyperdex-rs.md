...
| 50 | After the schema cleanup, the healthy routed put/get property should verify the exact remote-primary value too, because a presence-only assertion can miss remote attribute-mapping regressions. | Strengthen `hegel_distributed_runtime_routes_put_and_get` so the `runtime2` check asserts the exact `profile_views` value instead of only `Record(Some(_))`, then revalidate the targeted property, rerun `simulation-harness`, and rerun the full workspace. | `cargo test -p simulation-harness hegel_distributed_runtime_routes_put_and_get -- --nocapture` passes after the stronger primary-side assertion lands; `cargo test -p simulation-harness` passes; `cargo test --workspace` passes. | Confirmed. | advance | Extend Hegel coverage to routed numeric mutation so generated tests cover the distributed atomic-add path. |
