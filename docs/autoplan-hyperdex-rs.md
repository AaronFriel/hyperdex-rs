...
- [x] (2026-03-27 23:18Z) Relaunched the remaining post-follow reduction with
  two concrete owners: `Pauli` on a fresh product worktree and `Nietzsche` on
...
action after this pass is to launch one product pass plus one read-only
operation-order map, then use that pair to isolate which selected large-object
test or second operation hangs after setup.
