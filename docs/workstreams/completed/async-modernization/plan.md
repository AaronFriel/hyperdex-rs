...
- [x] (2026-03-29 10:10Z) Replaced tonic-generated async-trait service impls
  ...
  `e9bb387`.
- [x] (2026-03-29 18:05Z) Replaced the build-script rewrite path with the
  source-controlled `grpc-api` crate in `11275fd` and removed the remaining
  source-controlled `async-trait` dependency entries in `69020d9`.

## Current Hypothesis

This workstream is complete. The repository now uses a source-controlled gRPC
API crate instead of generated build-script rewrites, and source-controlled
`async-trait` usage is gone.

## Next Bounded Step

Keep this workstream completed unless a future tonic or transport redesign
