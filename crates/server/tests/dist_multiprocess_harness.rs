...
use legacy_protocol::{
    CountRequest, CountResponse, LegacyMessageType, RequestHeader, ResponseHeader,
};
use server::{request_coordinator_control_once, request_coordinator_control_with_body_once};
...
async fn coordinator_space_add_reaches_multiple_daemon_processes() -> Result<()> {
    ...
    daemon_one.wait_for_log("daemon synchronized coordinator config").await?;
    daemon_two.wait_for_log("daemon synchronized coordinator config").await?;
